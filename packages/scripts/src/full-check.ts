...
for (const [sutraId, data] of Object.entries(relationships.commentaries as Record<string, any>)) {
  if (data.commentaries) {
    for (const commentaryId of Object.keys(data.commentaries)) {
...
const excludePatterns = [
  ...
  /大般涅槃经后分/, /修行经/, /了本生死/, /稻芉/, /稻𦼮/,
  /舍黎娑/, /无边门/, /微密持/, /目佉尼/, /破魔陀罗尼/,
  /八吉祥/, /八阳/, /八部佛/, /佛名/, /神咒/, /咒经/,
  /缘生/, /缘起/, /本生/, /本行/, /本事/, /譬喻/,
  /因缘/, /感应/, /灵验/, /瑞应/, /应验/, /神异/,
]
...
for (const canon of canons) {
  ...
  for (const vol of vols) {
    ...
    for (const file of files) {
      ...
      if (result.isCommentary && result.type) {
        if (!unmatchedByCanon[canon]) unmatchedByCanon[canon] = []
        unmatchedByCanon[canon].push({ id, title, type: result.type })
        totalUnmatched++
      }
    }
  }
...
console.log('\n========================================')
console.log('注疏关系疏漏检查')
console.log('========================================')
console.log('\n未匹配的注疏总数:', totalUnmatched)
console.log('\n按藏分布:')
const sortedCanons = Object.entries(unmatchedByCanon).sort((a, b) => b[1].length - a[1].length)
for (const [canon, items] of sortedCanons) {
...
for (const [canon, items] of sortedCanons) {
  if (items.length > 0) {
    console.log('\n=== ' + canon + ' 藏未匹配注疏 (' + items.length + '个) ===')
    for (const item of items.slice(0, 30)) {
      console.log('  ' + item.id + ': ' + item.title + ' [' + item.type + ']')
    }
    if (items.length > 30) {
      console.log('  ... 还有 ' + (items.length - 30) + ' 个')
    }
  }
}
